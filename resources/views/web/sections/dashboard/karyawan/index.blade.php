...
<div class="grid lg:grid-cols-3 grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
    ...
    <div class="p-4 sm:col-span-full py-6 bg-white dark:bg-slate-800 shadow rounded-lg">
        <div class="mb-4 text-center">
            <h2 class="text-xl font-extrabold leading-none tracking-tight text-gray-900 md:text-3xl lg:text-4xl dark:text-white">Karyawan <span class="bg-blue-100 text-blue-800 text-2xl font-semibold mr-2 px-2.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-2">Beta</span></h2>
        ...
        editModal('Update Data', 'Simpan Perubahan')
            ...
            axios.get(`${editUrl}?id=${id}`)
                .then(response => {
                    ...
                    const kategoriIds = response.data.kategori_buku_relasi.map(relasi => relasi.toko_id);
                    // Set selected options in Select2 dropdown
                    $('.toko_id').val(kategoriIds).trigger('change');
