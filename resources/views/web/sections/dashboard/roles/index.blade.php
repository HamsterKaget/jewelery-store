...
<div class="grid lg:grid-cols-3 grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
    ...
    <div class="p-4 sm:col-span-full py-6 bg-white dark:bg-slate-800 shadow rounded-lg">
        <div class="mb-4 text-center">
            <h2 class="text-xl font-extrabold leading-none tracking-tight text-gray-900 md:text-3xl lg:text-4xl dark:text-white">Role</h2>
            <p class="text-lg max-w-4xl mx-auto font-normal text-gray-500 lg:text-xl dark:text-gray-400"></p>
        </div>
                ...
                <div class="relative w-full max-w-lg max-h-full">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="px-6 py-6 lg:px-8 rounded-lg">
                            <h3 id="titleSimpanData" class="mb-4 text-xl font-medium text-gray-900 dark:text-white" >Tambah Role</h3>
                            ...
                            >
                                ...
                                <div>
                                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Role / URL</label>
                                    <div class="flex">
                                ...
                                <div>
                                    <label for="guard_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Guard</label>
                                    <div class="flex">
                                        {{-- <input type="text" id="guard_name" name="guard_name" class="rounded-lg bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"> --}}
                                        {{-- <select name="guard_name" id="guard_name"> --}}
                                        <select id="guard_name" name="guard_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                            <option value="web" selected>web</option>
                                        </select>
                                    </div>
                                </div>
                                <div>
                                    <h3 class="mb-4 font-semibold text-gray-900 dark:text-white">Permission</h3>
                                    <ul id="permission-checkbox" class="w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg grid grid-cols-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                    </ul>
                                </div>
...
<script>
     ...
     *
     */
    function fetchData(params, pageUrl = "{{ route('role.get-data') }}") {
        axios.get(pageUrl, {
            params: params
        })
            .then(function (response) {
                ...
                `
                    <tr class="bg-white odd:bg-white even:bg-gray-50 text-center border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td>
                            ${item.permission && item.permission.length > 0 ? item.permission.map(permission => `<span class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">${permission.name}</span>`).join('') : '<span class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">Tidak ada permission</span>'}
                        </td>
    ...
    function permsData(params, pageUrl = "{{ route('permission.select') }}") {
        axios.get(pageUrl, {
            params: params
        })
            .then(function (response) {
                ...
                permissionCheckbox.innerHTML = ''; // Clear the existing content

                let index = 1;
                data.forEach(function (item) {
                    // Construct HTML using template literals
                    const liHTML = `
                        <li class="w-full border border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
                            <div class="flex items-center ps-3">
                                <input type="checkbox" value="${item.id}" id="checkbox-${index}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                <label for="checkbox-${index}" class="w-full py-3 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">${item.text}</label>
                            </div>
                        </li>
                    `;

                    // Append constructed HTML to the permissionCheckbox
                    permissionCheckbox.insertAdjacentHTML('beforeend', liHTML);
     ...
     */
     function submitData() {
        ...
        // Fetch checkbox values before resetting the form
        const checkedCheckboxes = document.querySelectorAll('#permission-checkbox input[type="checkbox"]:checked');

        checkedCheckboxes.forEach(checkbox => {
            formData.append('permissions[]', checkbox.value); // Append each checked checkbox value to formData
        });

        axios.post("{{ route('role.create') }}", formData)
            .then(response => {
                // Handle the response (e.g., show a success message)
                console.log('Data created successfully', response);
                fetchData(); // Refresh the table after creation
        ...
        // Reset form after fetching the checkbox values
        form.reset();
        createModal.hide();
    }
    ...
    function OperationModal(titleText, buttonText) {
    ...
    const form = document.getElementById('createForm');
    const editUrl = "{{ route('role.edit') }}";

    OperationModal('Update Data', 'Simpan Perubahan')
    ...
    axios.get(`${editUrl}?id=${id}`)
        .then(response => {
            ...
            inputField = form.querySelector('#name');
            inputField.value = response.data.name;

            inputField = form.querySelector('#guard_name');
            inputField.value = response.data.guard_name;

            // Mark checkboxes based on role permissions
            const permissions = response.data.permission.map(permission => parseInt(permission.id)); // Convert permissions to integers
            ...
            checkboxes.forEach(checkbox => {
                ...
                if (permissions.includes(parseInt(checkbox.value))) {
    ...
    function handleDeleteAction(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                .then(response => {
                    // Handle the response (e.g., show a success message)
                    console.log('Toko deleted successfully', response);
